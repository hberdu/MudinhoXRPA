...
- **Alvo de reset: 350** (`$TargetLevel`), que é o mínimo que o servidor exige — confirmado pela mensagem dele (`"Você precisa de estar no level 350 para resetar!"`). O `$LevelMinReset` acompanha.
  ...
  O A/B antigo (350 vs 380, `272925` contra `175938` pontos/h) sugere que alvo **menor** rende mais — o ciclo encurta mais do que os pontos por reset caem. Mas os dois braços rodaram em sequência, não intercalados, então a comparação não era controlada; o `pontos/h` do log é quem decide antes de mexer.
- O A/B automático está **desligado** (`$AutoTune = $false`) e, com ele desligado, o `alvo` gravado no `estado.txt` é ignorado — quem manda é o CONFIG, senão um alvo antigo do experimento sobrescreveria a escolha pra sempre.
