...
- **`/darmr` por validação**: só quando os 4 atributos = **32767** manda `/darmr`, vai à tela de login e re-entra. Depois o personagem volta em Lorencia: entra em **modo warmup** — usa `/losttower7` e farma lá até **3 resets** (`$WarmupResets`, distribuindo os pontos), depois volta ao `/s18` normal. Eram 10: medido, cada reset em Lost Tower custa ~220s contra 70-110s no Stadium pelos mesmos ~6200 pontos, então os 10 eram 73-78% do tempo do MR inteiro.
