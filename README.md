...
- **Alvo de reset: 350** (`$TargetLevel`), que é o mínimo que o servidor exige — confirmado pela mensagem dele (`"Você precisa de estar no level 350 para resetar!"`). O `$LevelMinReset` acompanha.
  Os dois são só **ponto de partida**: quem manda é o servidor, e o bot aprende dos **dois lados** — a mensagem de recusa sobe o piso, e um reset aceito de primeira abaixo dele o baixa. A cada `$ResetRetestResets` (20) resets ele tenta o alvo do CONFIG de novo, senão um piso aprendido ficaria gravado pra sempre e uma exigência que caísse nunca seria notada.
  O `minReset=` do `estado.txt` é restaurado **sempre** no load (diferente do `alvo=`, que só entra com `$AutoTune`), então trocar o CONFIG sem apagar aquela linha não adianta — o valor velho volta por cima.
  **Não subir esse alvo.** A razão é do jogo, não do bot: quanto **maior** o level, mais devagar ele sobe. Os levels entre 350 e um alvo maior são os mais caros do ciclo, e os pontos a mais que rendem no reset não pagam o tempo. Resetar no piso do servidor é o ciclo mais curto possível.
  Há um A/B antigo (350 vs 380, `272925` contra `175938` pontos/h) no histórico que parece dizer o contrário — **ignore**: os dois braços rodaram em *sequência*, não intercalados, então mediram noites diferentes, não o alvo. Se um dia isso for testado de novo, tem que ser intercalando reset a reset.
- O A/B automático está **desligado** (`$AutoTune = $false`), e não é "ainda não ligamos" — ligá-lo faria o bot gastar resets medindo um alvo maior que já se sabe pior. Com ele desligado, o `alvo` gravado no `estado.txt` é ignorado: quem manda é o CONFIG, senão um alvo antigo do experimento sobrescreveria a escolha pra sempre.
